...
import { useCMS, useForm, useLocalForm } from 'tinacms';
...
export default function Data() {
  ...
  const [page, form] = useForm({
    ...
    onSubmit(data, form) {
      alert(
        'Form submitted! Just like how we fetched content, we could also send it to an API from here.'
      );
      console.clear();
      console.log(data);
    },
  ...
  async function getContent() {
    try {
      let content = await fetch('https://httpbin.org/anything', {
        ...
        body: JSON.stringify({
          body: 'This data is coming from an API response!',
        }),
      });
...
return (
  ...
  return (
    <>
      <p>{page.text}</p>
      <Code
        readMoreLink="https://tinacms.org/docs/cms#adding-an-api"
        readMoreText="Tina provides additional API functionality"
      >
        {code}
      </Code>
    </>
